...
std::string getFilePrefix(const Rule &r) {
...
std::string dot(const Rule &r, const Options &options) {
...
bool disallowHydrogenCollapseByChangeThenCallback(const Rule &r, const CombinedVertex vCG,
...
struct AdvOptionsSide {
	AdvOptionsSide(const Rule &r, int idOffset, const BaseArgs &args,
...
std::string pdfCombined(const Rule &r, const Options &options) {
	MOD_ABORT;
}

std::pair<std::string, std::string> summary(const Rule &r, bool printCombined) {
	mod::graph::Printer first;
	mod::graph::Printer second;
...
std::string stereoSummary(const Rule &r, lib::DPO::CombinedRule::CombinedVertex vcg, Membership m,
